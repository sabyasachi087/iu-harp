...
public class MDSLauncher extends Configured implements Tool {

  public static void main(String[] argv) throws Exception {
    int res = ToolRunner.run(new Configuration(), new MDSLauncher(), argv);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    // 8 /N/u/zhangbj/hadoop-2.2.0/mds_data_split/data/ 
    // distance_ weight_ v_ 
    // /N/u/zhangbj/hadoop-2.2.0/mds_data_split/ids/distance_ids
    ...
    if (args.length != 13) {
      ...
      System.out.println("[11. CG iteration num]");
      System.out.println("[13. Num threads per mapper]");
      System.exit(0);
    }
    // Args
    ...
    System.out.println("[5. V File Prefix]:\t" + vPrefix);
    System.out.println("[6. IDs File ]:\t" + idsFile);
    System.out.println("[7. Label Data File ]:\t" + labelsFile);
    ...
    System.out.println("[12. CG Iterations]:\t" + cgIter);
    System.out.println("[13. Num threads per mapper]:\t" + numThreads);
    
    int jobCount = 0;
    int numJobs = 3;
    boolean jobSuccess = true;
    do {
      jobSuccess = launch(numMapTasks, inputFolder, inputPrefix, weightPrefix,
        vPrefix, idsFile, labelsFile, threshold, d, alpha, n, cgIter,
        numThreads);
      if (jobSuccess) {
        break;
      } else {
  ...
  boolean launch(int numMapTasks, String inputFolder, String inputPrefix,
    String weightPrefix, String vPrefix, String idsFile, String labelsFile,
    double threshold, int d, double alpha, int n, int cgIter, int numThreads)
    throws IOException, URISyntaxException, InterruptedException,
    ExecutionException, ClassNotFoundException {
    Configuration configuration = getConf();
    FileSystem fs = FileSystem.get(configuration);
    // Generate working directory, for partition files
    ...
    // Each file contains a data partition and a weight partition (or multiple)
    DataGen.generatePartitionFiles(inputFolder, inputPrefix, weightPrefix,
      vPrefix, dataDirPath, fs, numMapTasks);
    return runWDAMDS(numMapTasks, dataDirPath, xFilePath, xOutFilePath,
      outDirPath, idsFile, labelsFile, threshold, d, alpha, n, cgIter,
      numThreads);
  }

  private boolean runWDAMDS(int numMapTasks, Path dataDirPath, Path xFilePath,
    Path xOutFilePath, Path outDirPath, String idsFile, String labelsFile,
    double threshold, int d, double alpha, int n, int cgIter, int numThreads)
    throws IOException, URISyntaxException, InterruptedException,
    ClassNotFoundException {
    long jobStartTime = System.currentTimeMillis();
    Job mdsJob = prepareWDAMDSJob(numMapTasks, dataDirPath, xFilePath,
      xOutFilePath, outDirPath, idsFile, labelsFile, threshold, d, alpha, n,
      cgIter, numThreads);
    boolean jobSuccess = mdsJob.waitForCompletion(true);
    if (jobSuccess) {
      System.out.println("| Job finished in "
        + (System.currentTimeMillis() - jobStartTime) / 1000.0 + " seconds |");
      return true;
    } else {
  ...
  private Job prepareWDAMDSJob(int numMapTasks, Path dataDirPath,
    Path xFilePath, Path xOutFilePath, Path outDirPath, String idsFile,
    String labelsFile, double threshold, int d, double alpha, int n,
    int cgIter, int numThreads) throws IOException, URISyntaxException,
    InterruptedException, ClassNotFoundException {
    Job job = new Job(getConf(), "map-collective-wdamds");
    ...
    // Load from HDFS
    // Now we ignore and don't read x file from HDFS
