...
public class MDSDataSplit {
  public static void main(String[] args) {
    if (args.length != 8) {
      ...
      System.out.println("[5. Num of partitions]");
    ...
    try {
      if (choice == 0)
        splitDataforShort(dataFile, tmpDir, tmpFilePrefix, idsFile,
          numPartitions, row, width);
      else if (choice == 1)
        splitDataforDouble(dataFile, tmpDir, tmpFilePrefix, idsFile,
          numPartitions, row, width);
      else {
    ...
    System.out
      .println("==========================================================");
    System.out.println("Time to split data = " + (endTime - beginTime) / 1000
      + " Seconds.");
    System.out
      .println("==========================================================");
    System.exit(0);
  ...
  private static void splitDataforDouble(String dataFile, String tmpDir,
    String tmpFilePrefix, String idsFile, int numPartitions, int row, int width)
    throws IOException {
    ...
    for (int i = 0; i < numPartitions; i++) {
      System.out.println("The " + i + "th partition");
      idsWriter.write(i + "\t");
      ...
      for (int j = startRow; j < endRow; j++) {
        for (int k = 0; k < width; k++) { // Note the start
      ...
      idsWriter.write(curHeight + "\t" + width + "\t" + i + "\t" + startRow
        + "\n");
      writeToBinFile(rows, curHeight, width, outputFile);
      startRow = endRow;
    }
    din.close();
    idsWriter.flush();
  ...
  public static void writeToBinFile(double[][] row, int curHeight, int size,
  ...
  public static void writeToBinFile(short[][] row, int curHeight, int width,
    String fileName) throws IOException {
    DataOutputStream dout = new DataOutputStream(new BufferedOutputStream(
      new FileOutputStream(fileName)));
    for (int i = 0; i < curHeight; i++) {
