...
class DataGen {
  ...
  static boolean generateXData(int numDataPoints, int targetDim,
    Path xFilePath, FileSystem fs) throws IOException {
    ...
    double[] data = new double[numDataPoints * targetDim];
  ...
  /**
   * Partition file generation is very important. We need to verify it later.
   * The format in each partition file is: #ROWS Row ID ... #FILES Distance File
   * Weight File V File
   * 
   * @param inputFolder
   * @param inputPrefix
   ...
   * @return
   */
  public static boolean generatePartitionFiles(String inputFolder,
    String inputPrefix, String weightPrefix, String vPrefix, Path dataDirPath,
    FileSystem fs, int numMapTasks) {
    // Assume inputFolder is a place where all processes can access
    ...
    // One data file matches with one weight file and one v file
    if (files.length % 3 != 0) {
      return false;
    }
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      if (fileName.startsWith(inputPrefix)) {
        ...
        String weightFileName = fileName
          .replaceFirst(inputPrefix, weightPrefix);
        String vFileName = fileName.replaceFirst(inputPrefix, vPrefix);
        rowIDs.add(rowID);
        filePaths.add(inputDirPath + "/" + inputFileName);
        filePaths.add(inputDirPath + "/" + weightFileName);
        filePaths.add(inputDirPath + "/" + vFileName);
        count++;
        // The base count limit is numSetPerPartition,
        // I adjust it if there is still partitions left.
        ...
        if (count == countLimit) {
          ...
          Path partitionFilePath = new Path(dataDirPath, "partition_"
            + partitionID);
          try {
