 ...
 * is <em>never</em> made smaller (even on a {@link #clear()}). A family of
 * {@linkplain #trim() trimming methods} lets you control the size of the table;
 * this is particularly useful if you reuse instances of this class.
 * 
 * <p>
 * <strong>Warning:</strong> The implementation of this class has significantly
 * changed in <code>fastutil</code> 6.1.0. Please read the comments about this
 * issue in the section &ldquo;Faster Hash Tables&rdquo; of the <a
 * href="../../../../../overview-summary.html">overview</a>.
 * 
 * @see Hash
 * @see HashCommon
 */
public class Int2ObjectReuseHashMap<V> extends AbstractInt2ObjectMap<V>
  ...
  /** Add for recording V class @author zhangbj */
  ...
  public Int2ObjectReuseHashMap(final int expected, final float f,
    Class<V> vClass) {
    if (f <= 0 || f > 1)
    ...
    maxFill = maxFill(n, f);
    key = new int[n];
    // Allow value objects can be arrays
    // @author zhangbj
    value = (V[]) Array.newInstance(vClass, n);
    used = new boolean[n];
    // Add for recording value class type
    // @author zhangbj
    this.vClass = vClass;
  }
   ...
   */
  public Int2ObjectReuseHashMap(final int expected, Class<V> vClass) {
   ...
   */
  public Int2ObjectReuseHashMap(Class<V> vClass) {
   ...
   * @param f the load factor.
   */
  public Int2ObjectReuseHashMap(final Map<? extends Integer, ? extends V> m,
   ...
   * @param f the load factor.
   */
  public Int2ObjectReuseHashMap(final Int2ObjectMap<V> m, final float f,
    Class<V> vClass) {
    this(m.size(), f, vClass);
    putAll(m);
  }
   ...
   * @param m a type-specific map to be copied into the new hash map.
   */
  public Int2ObjectReuseHashMap(final Int2ObjectMap<V> m, Class<V> vClass) {
    this(m, DEFAULT_LOAD_FACTOR, vClass);
  }
   ...
   *           different lengths.
   */
  public Int2ObjectReuseHashMap(final int[] k, final V v[], final float f,
    Class<V> vClass) {
   ...
   *           different lengths.
   */
  public Int2ObjectReuseHashMap(final int[] k, final V v[], Class<V> vClass) {
  ...
  /**
   * Clean: To use value objects, not do memory allocation again, value object
   * is still leaved there. The method also returns the size of current map.
   * 
   * @author zhangbj
   */
  public int clean() {
   ...
   */
  private final class MapEntry implements Int2ObjectMap.Entry<V>,
    Map.Entry<Integer, V> {
  ...
  private class MapIterator {
     ...
     */
    int pos = Int2ObjectReuseHashMap.this.n;
    /**
    ...
    {
      final boolean used[] = Int2ObjectReuseHashMap.this.used;
      if (c != 0)
        while (!used[--pos])
          ;
    }
    ...
    protected final int shiftKeys(int pos) {
      ...
      for (;;) {
        pos = ((last = pos) + 1) & mask;
        while (used[pos]) {
          slot = (it.unimi.dsi.fastutil.HashCommon.murmurHash3((key[pos])
            ^ mask))
            & mask;
          if (last <= pos ? last >= slot || slot > pos : last >= slot
            && slot > pos)
    ...
    public void remove() {
      ...
      if (pos < -1) {
        ...
        Int2ObjectReuseHashMap.this.remove(wrapped.getInt(-pos - 2));
        last = -1;
        return;
      }
      size--;
  ...
  private class FastEntryIterator extends MapIterator implements
    ObjectIterator<Int2ObjectMap.Entry<V>> {
    final edu.iu.harp.util.IntBasicEntry<V> entry = new IntBasicEntry<V>(
      ((int) 0), (null));

    public edu.iu.harp.util.IntBasicEntry<V> next() {
  ...
  private final class MapEntrySet extends
    AbstractObjectSet<Int2ObjectMap.Entry<V>> implements FastEntrySet<V> {
    ...
    public boolean remove(final Object o) {
      ...
      while (used[pos]) {
        if (((key[pos]) == (k))) {
          Int2ObjectReuseHashMap.this.remove(e.getKey());
          return true;
        }
        pos = (pos + 1) & mask;
      }
    ...
    public void clear() {
      Int2ObjectReuseHashMap.this.clear();
    }
  }
  ...
  private final class KeySet extends AbstractIntSet {
    ...
    public boolean remove(int k) {
      ...
      Int2ObjectReuseHashMap.this.remove(k);
      return size != oldSize;
    }
  ...
  protected void rehash(final int newN) {
    ...
    final V newValue[] = (V[]) Array.newInstance(vClass, newN);
   ...
   */
  @SuppressWarnings("unchecked")
  public Int2ObjectReuseHashMap<V> clone() {
    Int2ObjectReuseHashMap<V> c;
    try {
      c = (Int2ObjectReuseHashMap<V>) super.clone();
  ...
  private void readObject(java.io.ObjectInputStream s)
    ...
    final V value[] = this.value = (V[]) Array.newInstance(vClass, n);
