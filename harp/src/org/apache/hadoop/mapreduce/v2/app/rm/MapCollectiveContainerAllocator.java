 ...
 */
public class MapCollectiveContainerAllocator extends
  MapCollectiveContainerRequestor implements ContainerAllocator {
  ...
  public MapCollectiveContainerAllocator(ClientService clientService,
    AppContext context) {
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    ResourceMgrDelegate resMgrDelegate = new ResourceMgrDelegate(
      new YarnConfiguration(conf));
    List<NodeReport> reports = resMgrDelegate.getNodeReports();
    resMgrDelegate.close();
    long end = System.currentTimeMillis();
    LOG.info("Get node inforamtion, use " + (end - start) + " milliseconds.");
    ...
    this.eventHandlingThread = new Thread() {
      ...
      public void run() {
        ...
        while (!stopped.get() && !Thread.currentThread().isInterrupted()) {
          try {
            event = MapCollectiveContainerAllocator.this.eventQueue.take();
  ...
  protected synchronized void heartbeat() throws Exception {
    ...
    if (recalculateReduceSchedule) {
      ...
      scheduleReduces(getJob().getTotalMaps(), completedMaps,
        ...
        mapResourceReqt, reduceResourceReqt, pendingReduces.size(),
  ...
  public void handle(ContainerAllocatorEvent event) {
    ...
    if (qSize != 0 && qSize % 1000 == 0) {
      LOG.info("Size of event-queue in MapCollectiveContainerAllocator is "
        + qSize);
    }
    int remCapacity = eventQueue.remainingCapacity();
    if (remCapacity < 1000) {
      LOG.warn("Very low remaining capacity in the event-queue "
        + "of MapCollectiveContainerAllocator: " + remCapacity);
    }
    try {
  ...
  protected synchronized void handleEvent(ContainerAllocatorEvent event) {
    ...
    if (event.getType() == ContainerAllocator.EventType.CONTAINER_REQ) {
      ...
      LOG.info("Task ID: " + reqEvent.getAttemptID().getTaskId().getId()
        + ", Hosts : " + hosts);
      JobId jobId = getJob().getID();
      int supportedMaxContainerCapability = getMaxContainerCapability()
        .getMemory();
      if (reqEvent.getAttemptID().getTaskId().getTaskType()
        .equals(TaskType.MAP)) {
        if (mapResourceReqt == 0) {
          // mapResourceReqt = reqEvent.getCapability().getMemory();
          // ZBJ, use map-collective memory settings,
          // if cannot get, use the one from mapreduce
          mapResourceReqt = Integer.parseInt(getConfig().get(
            "mapreduce.map.collective.memory.mb", "0"));
          if (mapResourceReqt == 0) {
            LOG.info("Cannot get map-collective memory settings,"
              + " replace with mapreduce memory settings.");
            mapResourceReqt = reqEvent.getCapability().getMemory();
          }
          eventHandler.handle(new JobHistoryEvent(jobId,
            new NormalizedResourceEvent(
              org.apache.hadoop.mapreduce.TaskType.MAP, mapResourceReqt)));
        ...
        LOG.info("Get rounded off memory: "
          + reqEvent.getCapability().getMemory());
        scheduledRequests.addMap(reqEvent);// maps are immediately scheduled
      } else {
        if (reduceResourceReqt == 0) {
          reduceResourceReqt = reqEvent.getCapability().getMemory();
          eventHandler
            .handle(new JobHistoryEvent(jobId, new NormalizedResourceEvent(
              org.apache.hadoop.mapreduce.TaskType.REDUCE, reduceResourceReqt)));
  ...
  private void preemptReducesIfNeeded() {
    ...
    if (scheduledRequests.maps.size() > 0) {
      ...
      int availableMemForMap = memLimit
        - ((assignedRequests.reduces.size() - assignedRequests.preemptionWaitingReduces
          .size()) * reduceResourceReqt);
      ...
      if (availableMemForMap < mapResourceReqt) {
        ...
        int preemptMem = Math.min(scheduledRequests.maps.size()
          * mapResourceReqt, premeptionLimit);

        int toPreempt = (int) Math
          .ceil((float) preemptMem / reduceResourceReqt);
  ...
  public void scheduleReduces(int totalMaps, int completedMaps,
    ...
    if (scheduledMaps == 0 && numPendingReduces > 0) {
      LOG.info("All maps assigned. " + "Ramping up all remaining reduces:"
        + numPendingReduces);
      scheduleAllReduces();
      return;
    }
    ...
    int netScheduledReduceMem = (scheduledReduces + assignedReduces)
      * reduceResourceReqt;
    ...
    int idealReduceMemLimit = Math.min(
    ...
    LOG.info("completedMapPercent " + completedMapPercent + " totalMemLimit:"
      + totalMemLimit + " finalMapMemLimit:" + finalMapMemLimit
      + " finalReduceMemLimit:" + finalReduceMemLimit + " netScheduledMapMem:"
      + netScheduledMapMem + " netScheduledReduceMem:" + netScheduledReduceMem);
  ...
  private class ScheduledRequests {
    ...
    private final Map<TaskAttemptId, ContainerRequest> maps = new LinkedHashMap<TaskAttemptId, ContainerRequest>();
    ...
    private void assign(List<Container> allocatedContainers) {
      ...
      while (it.hasNext()) {
        ...
        LOG.info("Assigning container " + allocated.getId() + " with priority "
    ...
    private void containerAssigned(Container allocated,
      ...
      if (LOG.isDebugEnabled()) {
        LOG
          .info("Assigned container (" + allocated + ") " + " to task "
            + assigned.attemptID + " on node "
  ...
  private class AssignedRequests {
    ...
    private final LinkedHashMap<TaskAttemptId, Container> reduces = new LinkedHashMap<TaskAttemptId, Container>();
    private final Set<TaskAttemptId> preemptionWaitingReduces = new HashSet<TaskAttemptId>();

    void add(Container container, TaskAttemptId tId) {
      LOG.info("Assigned container " + container.getId().toString() + " to "
        + tId);
    ...
    void preemptReduce(int toPreempt) {
      ...
      Collections.sort(reduceList, new Comparator<TaskAttemptId>() {
        ...
        public int compare(TaskAttemptId o1, TaskAttemptId o2) {
          float p = getJob().getTask(o1.getTaskId()).getAttempt(o1)
            .getProgress()
  ...
  private class ScheduleStats {
    ...
    public void updateAndLogIfChanged(String msgPrefix) {
      ...
      synchronized (MapCollectiveContainerAllocator.this) {
    ...
    public void log(String msgPrefix) {
      LOG.info(msgPrefix + "PendingReds:" + numPendingReduces
        + " ScheduledMaps:" + numScheduledMaps + " ScheduledReds:"
        + numScheduledReduces + " AssignedMaps:" + numAssignedMaps
        + " AssignedReds:" + numAssignedReduces + " CompletedMaps:"
        + numCompletedMaps + " CompletedReds:" + numCompletedReduces
        + " ContAlloc:" + numContainersAllocated + " ContRel:"
        + numContainersReleased + " HostLocal:" + hostLocalAssigned
