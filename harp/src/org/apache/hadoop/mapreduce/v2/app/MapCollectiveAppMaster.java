...
/**
 * Modified from MRAppMaster
 * 
 * The following is the original class description:
 * 
...
public class MapCollectiveAppMaster extends CompositeService {

	private static final Log LOG = LogFactory.getLog(MapCollectiveAppMaster.class);

	/**
	 * Priority of the MapCollectiveAppMaster shutdown hook.
	 */
	public static final int SHUTDOWN_HOOK_PRIORITY = 30;
	private static final byte[] SECRET_SHUFFLE_KEY = ("HARP_SECET_SHUFFLE" + System.currentTimeMillis()).getBytes();
	...
	private Job job;
	private Credentials jobCredentials = new Credentials(); // Filled during
															// init
	...
	public MapCollectiveAppMaster(ApplicationAttemptId applicationAttemptId, ContainerId containerId, String nmHost,
			int nmPort, int nmHttpPort, long appSubmitTime, int maxAppAttempts) {
		this(applicationAttemptId, containerId, nmHost, nmPort, nmHttpPort, new SystemClock(), appSubmitTime,
				maxAppAttempts);
	}

	public MapCollectiveAppMaster(ApplicationAttemptId applicationAttemptId, ContainerId containerId, String nmHost,
			int nmPort, int nmHttpPort, Clock clock, long appSubmitTime, int maxAppAttempts) {
		super(MapCollectiveAppMaster.class.getName());
		this.clock = clock;
		...
		LOG.info("Created MapCollectiveAppMaster for application " + applicationAttemptId);
	}
	...
	protected void serviceInit(final Configuration conf) throws Exception {
		...
		LOG.info("The specific max attempts: " + maxAppAttempts + " for application: "
				+ appAttemptID.getApplicationId().getId() + ". Attempt num: " + appAttemptID.getAttemptId()
		...
		// ZBJ, log map-collective job memory settings
		LOG.info("MapCollective options: " + conf.get("mapreduce.map.collective.memory.mb", "<missing conf1>") + " "
				+ conf.get("mapreduce.map.collective.java.memory.mb", "<missing conf2>"));
		...
		try {
			...
			if (!stagingExists) {
				...
				// A commit was started so this is the last time, we just need
				// to know
		...
		if (errorHappenedShutDown) {
			...
			if (copyHistory) {
				// Now that there's a FINISHING state for application on RM to
				// give AMs
				// plenty of time to clean up after unregister it's safe to
				// clean
				// staging
				// directory after unregistering with RM. So, we start the
				// staging-dir
			...
			// service to allocate containers from RM (if non-uber) or to fake
			// it
			// (uber)
			...
			if (copyHistory) {
				...
				// stopped
				// first.
				// This will guarantee that all history-events are flushed
				// before AM
				// goes
				// ahead with shutdown.
				// Note: Even though JobHistoryEventHandler is started last, if
				// any
				// component creates a JobHistoryEvent in the meanwhile, it will
				// be just
				// be
			...
			// Init ClientService separately so that we stop it separately,
			// since this
			// service needs to wait some time before it stops so clients can
			// know the
			// final states
			...
			// corresponding service to launch allocated containers via
			// NodeManager
	...
	private OutputCommitter createOutputCommitter(Configuration conf) {
		...
		if (newApiCommitter) {
			...
			org.apache.hadoop.mapreduce.v2.api.records.TaskAttemptId attemptID = MRBuilderUtils.newTaskAttemptId(taskID,
					0);
	...
	public void shutDownJob() {
		...
		try {
			...
			MapCollectiveAppMaster.this.stop();

			if (isLastAMRetry) {
				// Send job-end notification when it is safe to report
				// termination to
				...
				if (getConfig().get(MRJobConfig.MR_JOB_END_NOTIFICATION_URL) != null) {
					try {
						...
						// If unregistration fails, the final state is
						// unavailable. However,
						// at the last AM Retry, the client will finally be
						// notified FAILED
						// from RM, so we should let users know FAILED via
						// notifier as well
	...
	private class JobFinishEventHandler implements EventHandler<JobFinishEvent> {
		...
		public void handle(JobFinishEvent event) {
			// Create a new thread to shutdown the AM. We should not do it
			// in-line
	 ...
	 * 
	 * @param forcedState
	 *            a state to force the job into or null for normal operation.
	 * @param diagnostic
	...
	protected Speculator createSpeculator(Configuration conf, AppContext context) {
		...
		try {
			...
			Constructor<? extends Speculator> speculatorConstructor = speculatorClass
					.getConstructor(Configuration.class, AppContext.class);
	...
	protected TaskAttemptListener createTaskAttemptListener(AppContext context) {
		TaskAttemptListener lis = new TaskAttemptListenerImpl(context, jobTokenSecretManager, getRMHeartbeatHandler(),
				SECRET_SHUFFLE_KEY);
		return lis;
	}
	...
	private final class ContainerAllocatorRouter extends AbstractService
		...
		protected void serviceStart() throws Exception {
			if (job.isUber()) {
				...
				// ZBJ, use a different container allocator
				// with map-collective memory settings
				// and remove node/rack locality info in allocation
		...
		protected void serviceStop() throws Exception {
			try {
				if (isLastAMRetry) {
					...
					LOG.info("Skipping cleaning up the staging dir. " + "assuming AM will be retried.");
				}
			} catch (IOException io) {
	...
	protected void serviceStart() throws Exception {
		...
		for (AMInfo info : amInfos) {
			dispatcher.getEventHandler()
					.handle(new JobHistoryEvent(job.getID(),
		...
		DefaultMetricsSystem.initialize("MapCollectiveAppMaster");

		if (!errorHappenedShutDown) {
			// create a job event for job intialization
			JobEvent initJobEvent = new JobEvent(job.getID(), JobEventType.JOB_INIT);
			// Send init to the job (this does NOT trigger job execution)
			// This is a synchronous call, not an event through dispatcher. We
			// want
			// job-init to be done completely here.
			jobEventDispatcher.handle(initJobEvent);

			// JobImpl's InitTransition is done (call above is synchronous), so
			// the
			// "uber-decision" (MR-1220) has been made. Query job and switch to
			// ubermode if appropriate (by registering different
			// container-allocator
			// and container-launcher services/event-handlers).

			// ZBJ: Disable speculation for all the cases, no need for
			// mapcollective job
			speculatorEventDispatcher.disableSpeculation();
			if (job.isUber()) {
				// speculatorEventDispatcher.disableSpeculation();
				LOG.info("MapCollectiveAppMaster uberizing job " + job.getID()
						+ " in local container (\"uber-AM\") on node " + nmHost + ":" + nmPort + ".");
			} else {
				// send init to speculator only for non-uber jobs.
				// This won't yet start as dispatcher isn't started yet.
				dispatcher.getEventHandler().handle(new SpeculatorEvent(job.getID(), clock.getTime()));
				LOG.info("MapCollectiveAppMaster launching normal, non-uberized, multi-container " + "job "
						+ job.getID() + ".");
			}
	...
	private void processRecovery() {
		...
		if (recoveryEnabled && recoverySupportedByCommitter && (numReduceTasks <= 0 || shuffleKeyValidForRecovery)) {
			...
			LOG.info("Will not try to recover. recoveryEnabled: " + recoveryEnabled + " recoverySupportedByCommitter: "
					+ recoverySupportedByCommitter + " numReduceTasks: " + numReduceTasks
					+ " shuffleKeyValidForRecovery: " + shuffleKeyValidForRecovery + " ApplicationAttemptID: "
	...
	private static FSDataInputStream getPreviousJobHistoryStream(Configuration conf, ApplicationAttemptId appAttemptId)
			throws IOException {
		...
		try {
			...
			while ((event = jobHistoryEventReader.getNextEvent()) != null) {
				if (event.getEventType() == EventType.AM_STARTED) {
					...
					amInfos.add(MRBuilderUtils.newAMInfo(amStartedEvent.getAppAttemptId(),
							amStartedEvent.getStartTime(), amStartedEvent.getContainerId(),
							StringInterner.weakIntern(amStartedEvent.getNodeManagerHost()),
							amStartedEvent.getNodeManagerPort(), amStartedEvent.getNodeManagerHttpPort()));
	 ...
	 * job stuff has been moved to init() above to support uberization
	 * (MR-1220). In a typical workflow, one presumably would want to uberize
	 * only a subset of the jobs (the "small" ones), which is awkward with the
	 * current design.
	 */
	@SuppressWarnings("unchecked")
	...
	private class SpeculatorEventDispatcher implements EventHandler<SpeculatorEvent> {
		private final Configuration conf;
		...
		public void handle(SpeculatorEvent event) {
			...
			/*
			 * The point of the following is to allow the MAP and REDUCE
			 * speculative config values to be independent: IF spec-exec is
			 * turned on for maps AND the task is a map task OR IF spec-exec is
			 * turned on for reduces AND the task is a reduce task THEN call the
			 * speculator to handle the event.
			 */
	...
	public static void main(String[] args) {
		try {
			...
			MapCollectiveAppMaster appMaster = new MapCollectiveAppMaster(applicationAttemptId, containerId,
					nodeHostString, Integer.parseInt(nodePortString), Integer.parseInt(nodeHttpPortString),
					appSubmitTime, Integer.parseInt(maxAppAttempts));
			ShutdownHookManager.get().addShutdownHook(new MapCollecitveAppMasterShutdownHook(appMaster),
			...
			//conf.set(HttpConfig.Policy., value);
			//HttpConfig.setPolicy(HttpConfig.Policy.HTTP_ONLY);
			String jobUserName = System.getenv(ApplicationConstants.Environment.USER.name());
			conf.set(MRJobConfig.USER_NAME, jobUserName);
			// Do not automatically close FileSystem objects so that in case of
			// SIGTERM I have a chance to write out the job history. I'll be
			// closing
			// the objects myself.
			...
			int maxMapAttempt = conf.getMaxMapAttempts();
			LOG.info("Num Map Tasks " + numMapTasks + ", Max map attempt: " + maxMapAttempt);
			initAndStartAppMaster(appMaster, conf, jobUserName);
		} catch (Throwable t) {
			LOG.fatal("Error starting MapCollectiveAppMaster", t);
			System.exit(1);
		}
	}
	...
	// close of the JVM.
	static class MapCollecitveAppMasterShutdownHook implements Runnable {
		MapCollectiveAppMaster appMaster;

		MapCollecitveAppMasterShutdownHook(MapCollectiveAppMaster appMaster) {
			this.appMaster = appMaster;
		}

		public void run() {
			LOG.info("MapCollectiveAppMaster received a signal. Signaling RMCommunicator and "
					+ "JobHistoryEventHandler.");

			// Notify the JHEH and RMCommunicator that a SIGTERM has been
			// received so
	...
	protected static void initAndStartAppMaster(final MapCollectiveAppMaster appMaster, final JobConf conf,
			String jobUserName) throws IOException, InterruptedException {
		UserGroupInformation.setConfiguration(conf);
		// Security framework already loaded the tokens into current UGI, just
		// use
		// them
