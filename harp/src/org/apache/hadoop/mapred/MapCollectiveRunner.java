 ...
 * submitJob, launch MapCollectiveAppMaster
 */
@SuppressWarnings("unchecked")
public class MapCollectiveRunner implements ClientProtocol {

	private static final Log LOG = LogFactory.getLog(MapCollectiveRunner.class);
	 ...
	 */
	public MapCollectiveRunner(Configuration conf, ResourceMgrDelegate resMgrDelegate) {
		this(conf, resMgrDelegate, new ClientCache(conf, resMgrDelegate));
	}

	/**
	 * Similar to
	 * {@link MapCollectiveRunner#YARNRunner(Configuration, ResourceMgrDelegate)}
	 ...
	 */
	public MapCollectiveRunner(Configuration conf, ResourceMgrDelegate resMgrDelegate, ClientCache clientCache) {
		this.conf = conf;
		try {
	...
	void addHistoryToken(Credentials ts) throws IOException, InterruptedException {
		...
		if (UserGroupInformation.isSecurityEnabled() && (hsProxy != null)) {
			...
			// --Commented as per 2.7v and line 200 added
			/*
	...
	public JobStatus submitJob(JobID jobId, String jobSubmitDir, Credentials ts)
		...
		LOG.info("Using MapCollectiveRunner");
		// No checking at the current stage
		// checkClusterResourceAvailability()
	...
	private void checkClusterResourceAvailability() throws IOException {
		try {
			int numSlots = resMgrDelegate.getYarnClusterMetrics().getNumNodeManagers();
			LOG.info("Num of nodes: " + numSlots);
			int mapMem = Integer.parseInt(conf.get("mapreduce.map.collective.memory.mb", "0"));
			if (mapMem == 0) {
				mapMem = Integer.parseInt(conf.get("mapreduce.map.memory.mb", "0"));
			}
			if (mapMem == 0) {
				throw new YarnException("Invalid map memory settings.");
			}
			LOG.info("Map mem: " + mapMem);
			List<NodeReport> nodes = resMgrDelegate.getNodeReports();
			int totalMem = 0;
			for (NodeReport node : nodes) {
				totalMem = totalMem + node.getCapability().getMemory();
				LOG.info("Node " + node.getNodeId().getHost() + ", Mem: " + node.getCapability().getMemory());
			}
			org.apache.hadoop.mapred.JobConf jobConf = (org.apache.hadoop.mapred.JobConf) conf;
			int numMapTasks = jobConf.getNumMapTasks();
			if (mapMem * numMapTasks > totalMem) {
				throw new YarnException("Exceeded cluster resource capacity.");
			}
		} catch (YarnException e) {
			throw new IOException("Failed to run job. Exceeded cluster resource capacity.");
		}
	}
	...
	public ApplicationSubmissionContext createApplicationSubmissionContext(Configuration jobConf, String jobSubmitDir,
		...
		// Added for2.7v
		MRApps.addLog4jSystemProperties(null, vargs, jobConf);
		...
		// ZBJ: No fefault application master class but a customized one
		// vargs.add(MRJobConfig.APPLICATION_MASTER_CLASS);
		vargs.add("org.apache.hadoop.mapreduce.v2.app.MapCollectiveAppMaster");
